...
export function CustomerCategoryFilter({ isOpen, onClose, onApplyFilters, currentFilters }) {
    ...
    const toggleStatus = (key) => {
        setFilters(prev => ({ ...prev, status: { supprime: false, [key]: !prev.status[key] } }));
    };

    const resetFilters = () => {
        const reset = { status: { supprime: false } };
        setFilters(reset);
        ...
        <div ref={dropdownRef} className="absolute right-0 top-full mt-2 w-80 futuristic-card rounded-2xl shadow-2xl py-4 z-50 max-h-[80vh] overflow-y-auto">
            ...
            <div className="relative z-10 flex items-center justify-between px-4 pb-3 border-b border-slate-600/50">
                <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-lg font-semibold text-white">Filtrer les catégories</h3>
                </div>
