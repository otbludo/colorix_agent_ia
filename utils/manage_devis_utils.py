    ...
    async def validate_devis(self, devis_data: DevisValidate, current_user):
        try:
            ...
            if devis.status == "valide":
                return {
                    "message": "Devis était déjà validé. Facture non regénérée.",
                    "devis": devis
                }
            ...
            await self.create_audit_log(
                object_id=devis.id,
                action=f"Devis validé et facture générée ({pdf_path})",
                performed_by=admin_id,
                performed_by_email=current_user["email"]
            )

            return {
                "message": "Devis validé et facture PDF générée avec succès",
                "devis": devis,
                "pdf_path": pdf_path
            }
